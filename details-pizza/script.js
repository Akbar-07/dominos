        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            function setActive(option) {
                pizzaOptions.forEach(opt => {
                    ...
                    if (svg) svg.style.display = "none";  // мы скрываем все
                    opt.classList.remove("active");
                });

                const svg = option.querySelector("svg");
                if (svg) svg.style.display = "inline-block"; // мы показываем только избранные
                option.classList.add("active");
            }

            // --- сделать средний div (data-id=3) активным по умолчанию при загрузке страницы ---
            const defaultOption = document.querySelector('.pizza_size_select[data-id="2"]');
            if (defaultOption) {
                setActive(defaultOption);
            }

            // --- сделать этот div активным при щелчке по другому div ---
...
// Ингредиенты по умолчанию (олинган из data-ids)
...
// НОВАЯ ДОБАВЛЕННАЯ ФУНКЦИЯ: Выбор ингредиентов по умолчанию на основе их номера
...
function updateTotalPrice() {
    ...
    // Поиск активного размера пиццы
    const activePizzaSize = document.querySelector(".pizza_size_select.active");
    let currentAksiyaPrice = aksiyaBasePrice; // значение по умолчанию
...
// Функция отображения выбранных ингредиентов в HTML
function updateIngredientsDisplay() {
    let displayContainer = document.querySelector('.container-map-info-price');

    // Если контейнер не существует, создайте его.
    if (!displayContainer) {
        displayContainer = document.createElement('div');
        displayContainer.className = 'container-map-info-price';

        // Найдите подходящее место для добавления контейнера (перед кнопкой addToCart)
    ...
    // Показать основную информацию о пицце
    ...
    // Показать дополнительные ингредиенты
...
// Максимум дополнительных звуковых ингредиентов
const maxExtraIngredients = 5;

// --- НОВЫЕ ФУНКЦИИ РЕДАКТИРОВАНИЯ ---

// Функция проверки статуса редактирования и восстановления данных
...
function restoreEditData() {
    ...
    // СНАЧАЛА ПОЛНОСТЬЮ ОЧИСТИТЕ ВСЕ КЛАССЫ
    ...
    // Избранные ингредиенты и начинки для пиццы
    ...
    // 2. Восстановить размер пиццы по ID
    if (editOrderData.pizzas && editOrderData.pizzas[0]) {
        ...
        // Удалить активное из всех размеров пиццы
        ...
        // Сначала выполните поиск по идентификатору
        ...
        // Если не удалось найти по идентификатору, выполните поиск по названию (обратная совместимость)
    ...
    // 3. ВСЕГДА добавляйте тесто и края по умолчанию
    setIngredient(defaultDough);
    setIngredient(defaultEdge);
    updateDoughSelection();
    
    // 4. Восстановить ингредиенты по ID
    if (editOrderData.pizzas && editOrderData.pizzas[0] && editOrderData.pizzas[0].ingredients) {
        ...
        // Определить ингредиенты по умолчанию по номеру в editOrderData.ingredients
        const ingredientsNumber = editOrderData.ingredients;
        
        let defaultIngredientsForThisProduct = [];
        
        // Выбор правильных ингредиентов по умолчанию на основе номера ингредиента
        ...
        // Группировать по идентификаторам
        const savedStandardIngredientIds = [];
        const savedExtraIngredients = [];
        
        savedIngredients.forEach(savedItem => {
            // Если у savedItem есть идентификатор, используйте его.
            if (savedItem.id) {
                // Проверьте, входит ли он в стандартные ингредиенты по идентификатору
                ...
                // Если идентификатор отсутствует, выполните поиск по имени (обратная совместимость)
                const ingredientName = savedItem.name.toLowerCase().trim();
                
                // Поиск по названию в стандартных ингредиентах по умолчанию
        ...
        // ВОССТАНОВИТЬ СТАНДАРТНЫЕ ИНГРЕДИЕНТЫ ПО ИДЕНТИФИКАТОРУ
        if (savedStandardIngredientIds.length > 0) {
            // Активировать только стандартные ингредиенты с сохраненными идентификаторами
            ...
            // Если нет сохраненного стандартного ингредиента,
            // активировать все настройки по умолчанию
        ...
        // ВОССТАНОВИТЬ ДОПОЛНИТЕЛЬНЫЕ ИНГРЕДИЕНТЫ ПО ИДЕНТИФИКАТОРУ
        savedExtraIngredients.forEach(savedItem => {
            ...
            // 1) Сначала выполните поиск по идентификатору
            if (itemId) {
                extraElement = document.querySelector(`.for_vibor_0[data-id="${itemId}"]`);
            }
            
            // 2) Если не найдено по идентификатору, искать по названию
            if (!extraElement && itemName) {
                // поиск по атрибуту data-title
                extraElement = document.querySelector(`.for_vibor_0[data-title="${itemName}"]`);
                
                // Если не удалось найти по названию данных, выполните поиск путем сравнения текстов.
                if (!extraElement) {
                ...
                // Если найден, добавьте к нему идентификатор.
            ...
            if (extraElement) {
                ...
                // Создайте дополнительный ингредиент
        ...
        // Если сохраненных ингредиентов нет, установите значения по умолчанию.
...
// Функция инициализации страницы также нуждается в небольшой доработке.
function initializePage() {
    // Заполнение элементов пользовательского интерфейса
    ...
    // Проверить статус редактирования
    const isEditRestored = restoreEditData();
    
    if (!isEditRestored) {
        // Загружать данные по умолчанию только в том случае, если режим редактирования не активен.
        
        // Сначала очистите все классы (даже для обычного режима)
        document.querySelectorAll(".for_active_0, .for_active_2, .vibor_active").forEach(el => {
            el.classList.remove("for_active_0", "for_active_2", "vibor_active");
        });
        ...
        // 1) Тесто по умолчанию было расположено краем вперед
        setIngredient(defaultDough);
        setIngredient(defaultEdge);
        updateDoughSelection();

        // 2) Стандартные ингредиенты по умолчанию
        defaultStandardIngredients.forEach(item => {
            setIngredient(item);
            const element = document.querySelector(`.for_vibor_2[data-id="${item.id}"]`);
            if (element) {
    ...
    // Настройка прослушивателей событий (всегда требуется)
    setupEventListeners();
    
    // Управляйте видимостью ингредиентов
...
// Обработчик дополнительных ингредиентов с идентификатором
function handleExtraIngredientClick() {
    // Получить ID — сначала сгенерировать из data-id, затем из title
    let id = this.dataset.id;
    const title = this.dataset.title;
    
    // Если data-id не существует, сгенерируйте ID из заголовка
    if (!id && title) {
...
// Улучшение генератора идентификаторов дополнительных ингредиентов
function generateExtraIngredientId(title) {
    if (!title || typeof title !== 'string') {
        return "extra_unknown_" + Date.now();
    }
    
    // Очистить заголовок и создать идентификатор
    const cleanTitle = title.toLowerCase()
        .replace(/\s+/g, '_')           // заменить пробелы на _
        .replace(/[^a-z0-9_]/g, '')     // оставив только буквы, цифры и _
        .substring(0, 20);              // Сократить до 20 символов
...
function handleStandardIngredientClick() {
    ...
    // Работа только с кликабельными элементами
    ...
    // Если не выбраны ингредиенты по умолчанию, добавьте их заново.
    const hasAnyDefault = allDefaultIds.some(defId => activeIds.includes(defId));

    if (!hasAnyDefault) {
        // Нет ингредиентов по умолчанию, включите их снова
...
// Функция установки прослушивателей событий
function setupEventListeners() {
    // Прослушиватели событий дополнительных ингредиентов (for_vibor_0)
    ...
    // Стандартные прослушиватели событий ингредиентов
    const standardItems = document.querySelectorAll(".for_vibor_2");
...
// Функция добавления идентификатора к элементам дополнительных ингредиентов
function initializeExtraIngredientsIds() {
    ...
    extraElements.forEach((element, index) => {
        // Если data-id не существует, добавьте его
...
// Добавлять идентификаторы дополнительных ингредиентов при загрузке страницы

...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Если адрес не существует, мы перенаправим на страницу добавления адреса.
    if (locations.length === 0) {
        window.location.href = "../add-address/";
        return;
    }

    // Добавьте заголовок (как в исходном HTML)
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    container.innerHTML = "";
        ...
        if (locations.length === 0) {
        window.location.href = "../add-address/";
        return; // так что следующий код не работает
    }
    ...
    document.querySelectorAll(".edit-icon").forEach(icon => {
        icon.addEventListener("click", (e) => {
            const idx = e.currentTarget.getAttribute("data-index");
            const selectedAddress = locations[idx];
            // записываем выбранный адрес во временную сессию
            sessionStorage.setItem("editAddress", JSON.stringify(selectedAddress));
            window.location.href = "../address-details/";
        });
    });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Успешное чтение из SessionStorage
  ...
  if (success === 'true') {
    ...
    // 2 скрыть тост через несколько секунд и удалить успешно
    setTimeout(() => {
      toast.style.setProperty('display', 'none', 'important');
